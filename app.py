    ...
    while True:
        ...
        for entry in data:
            try:
                ...
                response = claim_request(url, entry['proxy'], entry['userid'])
                logging.info(f"{entry['discription']} join function : {response.status_code}")
            except Exception as e:
                logging.error(f"Error during claim request: {e}")
        time.sleep(6 * 60 * 60)  # Спим 6 часов (6 * 60 минут * 60 секунд)
